function Graph(b,c,a){this.canvas=null;this.context=null;this.lines=[];this.width=0;this.height=0;this.drawn=false;this.options={minValue:"auto",maxValue:"auto",padding:20,spacing:10,border:1,borderColor:"#ddd",background:["#fff","#f8f8f8"],grid:true,gridSize:0.5,gridColor:"rgb(150, 150, 150 )",gridShadow:false,gridDotted:true,axisSize:"auto",axisColor:"auto",axisShadow:"auto",bullets:true,bulletSize:5,bulletColor:"#000",bulletFill:false,bulletShadow:false,shadowColor:"rgba(0, 0, 0, 0.6)",fill:true,fillColor:"rgba(138,184,125,0.3)",lineSize:3,lineCurve:true,lineColor:"#8AB87D",useOffset:true,valueKey:"value"};
...
}};Graph.prototype.getLines=function(){return this.lines;};Graph.prototype.clearLines=function(){this.lines=null;delete this.lines;this.lines=[];};Graph.prototype.draw=function(){if(this.options.background!==false){this.drawBackground();}if(this.options.grid){this.drawGrid();}for(var b=0;b<this.lines.length;
b++){this.lines[b]["bullets"]=this.calculateBulletPoints(this.lines[b]["data"]);var a=this.calculateControlPoint(this.lines[b]["bullets"]);this.lines[b]["start"]=a.start;this.lines[b]["end"]=a.end;if(this.options.fill){this.fillLineData(this.lines[b],this.lines[b]["fillColor"]=="default"?this.options.fillColor:this.lines[b]["fillColor"]);
}var c=(!this.options.fill?this.lines[b]["lineColor"]:"default");var c=this.lines[b]["lineColor"]=="default"?this.options.lineColor:this.lines[b]["lineColor"];this.drawLineData(this.lines[b],c);if(this.options.bullets){this.drawBulletPoints(this.lines[b],c);}}this.drawn=true;};Graph.prototype.drawBackground=function(){if(this.options.border>0){this.context.fillStyle=this.parseColor(this.options.borderColor);
...
this.context.closePath();this.context.fill();if(this.options.bulletFill){this.context.stroke();}}};Graph.prototype.drawLineData=function(h,f){var d=h.bullets;var m=this.options.padding+(this.options.border>0?this.options.border:0);var l=this.options.spacing;var j=m;var i=this.height-m-l;var n=this.height-(m*2)-(l*2);
var b=this.width-(m*2);var c=d.length;var f=f||"default";this.context.beginPath();this.context.moveTo(d[0].x,d[0].y);if(this.options.gridShadow){this.context.shadowOffsetX=0;this.context.shadowOffsetY=0;this.context.shadowBlur=this.options.lineSize-1;this.context.shadowColor=this.options.shadowColor;
}else{this.context.shadowBlur=0;}this.context.strokeStyle=this.parseColor(f=="default"?this.options.lineColor:f);this.context.lineWidth=this.options.lineSize;this.context.lineCap="round";this.context.lineJoin="bevel";for(var e=0;e<d.length;e++){var a=d[e];var g=typeof(d[e-1])!="undefined"?d[e-1]:a;var k=typeof(d[e+1])!="undefined"?d[e+1]:a;
if(this.options.lineCurve&&c<3){this.options.lineCurve=false;}if(!this.options.lineCurve){this.context.lineTo(a.x,a.y);}else{if(e==(d.length-1)){break;}if(e==0){this.context.moveTo(a.x,a.y);}this.context.bezierCurveTo(h.start[e].x,h.start[e].y,h.end[e].x,h.end[e].y,k.x,k.y);}}this.context.stroke();};
Graph.prototype.fillLineData=function(h,f){var d=h.bullets;var l=this.options.padding+(this.options.border>0?this.options.border:0);var j=l;var i=this.height-l;var m=this.height-(l*2);var b=this.width-(l*2);var c=d.length;this.context.beginPath();this.context.moveTo(j,i);this.context.shadowBlur=0;for(var e=0;
e<d.length;e++){var a=d[e];var g=typeof(d[e-1])!="undefined"?d[e-1]:a;var k=typeof(d[e+1])!="undefined"?d[e+1]:a;if(!this.options.lineCurve){this.context.lineTo(a.x,a.y);}else{if(c<3){alert("[graph] cannot curve lines with less than 3 bullets points.");return false;}else{if(e==(d.length-1)){break;}}if(e==0){this.context.moveTo(a.x,a.y);
}this.context.bezierCurveTo(h.start[e].x,h.start[e].y,h.end[e].x,h.end[e].y,k.x,k.y);}}this.context.lineTo(j+b,i);this.context.lineTo(j,i);this.context.fillStyle=f;this.context.fill();};Graph.prototype.getMinimum=function(){var b=[];for(var a=0;a<this.lines.length;a++){b.push(Math.min.apply(Math,this.lines[a]["data"]));
...
var d=e.length;var c=false;var b=this.width/(d-1);for(var f=0;f<d;f++){var a=e[f];var i=f==0?0:a.x-b;var k=Math.floor((a.x-i)/2)-(this.options.padding/2);if(j<i||j>a.x){continue;}if(j<=(i+k)){c=f==0?a:e[f-1];c.meta=f==0?m[f]:m[f-1];break;}else{c=a;c.meta=m[f];break;}}h.push(c);this.context.strokeStyle=this.parseColor(this.lines[l]["lineColor"]=="default"?this.options.lineColor:this.lines[l]["lineColor"]);
